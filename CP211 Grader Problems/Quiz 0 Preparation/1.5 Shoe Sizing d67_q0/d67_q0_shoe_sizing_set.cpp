...
int main() {
	...
	for (int i = 0 ; i < count_store_size ; i++) { 
	    ...
	    store_size.insert(each_store_size);
	}
	...
	for (int j = 0 ; j < count_customer_size ; j++) {
		...
		// note : set use set.lower_bound is faster that lower_bound algorithm
	    auto best_fit_size_itr = store_size.lower_bound(each_customer_size) ; 
	    ...
	    // case 2 : biggest size possible
	        ...
	        cout << *prev(best_fit_size_itr) << " " ; // note : end dont have value , so step back
	    }
	    
	    // case 3 : general case , check distancee of best fit (left & right)
	    else { 
	        // if having >= 2 best size (i.e. 60 (store) < 90 (customer) < 120 (store) , giving bigger size)
	        ...
	        auto smaller_best_fit_itr = prev(best_fit_size_itr) ; 
