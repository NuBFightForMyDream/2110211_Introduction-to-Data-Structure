...
int main() {
    ...
    // store data in set 
    vector< pair<int , int> > year_month_published ; 
    for (int i = 0 ; i < count_published ; i++) { 
        int y , m ; cin >> y >> m ;
        pair<int , int> data = make_pair(y,m) ; 
        year_month_published.push_back( data ) ; 
    }
    
    // set will sort automatically (now it's vector)
    sort( year_month_published.begin() , year_month_published.end() ) ; 
